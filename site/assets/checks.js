...
(function () {
  ...
  function calcWbl() {
    ...
    const Dbl = k * rho * Vbl;
    const Pbl = 4865.4; // Dбл − Dпор − экипаж и 10 % запасов (по методике)
  ...
  /* ---------- 2. таблицы нагрузки ----------
   * [№, статья, P, xg, zg, Pxg, Pzg] — моменты как в методике */
  const LOAD_FULL = [
  ...
  function loadTable(rows, tableId, badgesId, target) {
    ...
    document.getElementById(badgesId).innerHTML =
      badgeRow(`самопроверка: ΣP = ${fmt(SP)} т (в методике ${fmt(target.D)}),
  ...
  function trim(loadFull, loadBal) {
    ...
    o.push(badgeRow(`в балласте: осадка кормой T<sub>к</sub> = ${fmt(Bl.Tk, 2)} м обеспечивает требуемое
      погружение винта 1,05·d<sub>в</sub> = 5,04 м (с точностью построений; в методике принято выполненным)`,
      Bl.Tk >= 5.04 * 0.99));
  ...
  function all() {
    ...
    document.getElementById('ck-load2-note').innerHTML = lb.stars.length
      ? `* В строках ${lb.stars.join(', ')} моменты приведены как в методике курсового проекта: при переходе
